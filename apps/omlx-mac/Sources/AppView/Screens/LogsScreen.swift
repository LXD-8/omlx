...
private enum LogTypeScale {
    /// Time, level and module columns.
    static let meta: CGFloat = 12
    /// The message itself; the app's body size.
    static let body: CGFloat = 13
    /// The detail card's record box: tall enough for a dozen lines, scrolled
    /// past that instead of growing with the record.
    static let detailBodyHeight: CGFloat = 200
        ...
        ZStack {
            if vm.rows.isEmpty {
                ...
                ScrollViewReader { proxy in
                    ...
                    .onChange(of: vm.rows.last?.id) { _, id in
                        guard vm.autoScroll, let id else { return }
...
private struct LogDetailCard: View {
    let row: LogRow
    let onClose: () -> Void
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            // Unbounded, a parameter dump made it 300,000pt tall and pushed the
            // list off the screen.
            ScrollView {
                Text(row.record.renderedFullMessage)
                    .font(.omlxMono(LogTypeScale.meta))
            ...
            .frame(maxWidth: .infinity, alignment: .leading)
            .frame(maxHeight: LogTypeScale.detailBodyHeight, alignment: .top)
            .padding(8)
...
/// all read the same mapping — and it is the same six the web console paints
/// (`logLevelTone` in `omlx/admin/static/js/dashboard.js`), so a WARNING is the
/// same colour in the window and in the browser.
///
