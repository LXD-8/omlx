...
final class MenubarControllerPortTests: XCTestCase {
    ...
    // login form. The key is traded for a short-lived token first, so only
    // the token reaches this URL builder. The action method itself needs a
    // live NSStatusBar, so we test the pure URL builder it delegates to.

    func testWebAdminURLUsesAutoLoginWithRedirect() throws {
        let url = try XCTUnwrap(
            MenubarController.webAdminURL(host: "127.0.0.1", port: 8000, authToken: "secret")
        )
        ...
        XCTAssertEqual(items.first { $0.name == "auth_token" }?.value, "secret")
        XCTAssertNil(items.first { $0.name == "key" },
                     "the permanent API key must never appear in the URL")
    }

    func testWebAdminURLBuildsIPv6Host() throws {
        let url = try XCTUnwrap(
            MenubarController.webAdminURL(host: "[::1]", port: 8000, authToken: nil)
        )
    ...
    func testWebAdminURLPercentEncodesToken() throws {
        ...
        let comps = try XCTUnwrap(URLComponents(url: url, resolvingAgainstBaseURL: false))
        XCTAssertEqual(comps.queryItems?.first { $0.name == "auth_token" }?.value, "a+b/c&d")
        // And the raw URL string carries the encoded form, not the literal.
        XCTAssertTrue(url.absoluteString.contains("auth_token=a%2Bb/c%26d"),
    ...
    func testWebAdminURLOmitsTokenWhenMissing() throws {
        for token in [nil, ""] as [String?] {
            let url = try XCTUnwrap(
                MenubarController.webAdminURL(host: "127.0.0.1", port: 8000, authToken: token)
            )
            let comps = try XCTUnwrap(URLComponents(url: url, resolvingAgainstBaseURL: false))
            XCTAssertNil(comps.queryItems?.first { $0.name == "auth_token" },
                         "empty/nil token must not emit an auth_token= param (server redirects to login instead)")
