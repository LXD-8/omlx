...
is what ``tests/test_design_tokens.py`` runs.
...
BANNER = "/* Generated by omlx/admin/build_tokens.py from omlx/admin/tokens.json.\n" \
...
# ``admin/routes.py``'s validator), and tests/test_design_tokens.py keeps the
# three lists in step.
...
def _theme_vars(tokens: dict, appearance: str) -> list[tuple[str, str]]:
    ...
    for name, value in label.items():
        if name.startswith("_"):
            continue
        pairs.append((f"--label-{name[:-2]}-fg", value) if name.endswith("Fg") else (f"--label-{name}", value))
    return pairs
...
def _geometry_vars(tokens: dict) -> list[tuple[str, str]]:
    pairs = [(f"--space-{name}", f"{value}px") for name, value in tokens["space"].items() if not name.startswith("_")]
    pairs += [(f"--radius-{name}", f"{value}px") for name, value in tokens["radius"].items()]
...
def _render_block(pairs: list[tuple[str, str]], indent: str = "    ") -> str:
    ...
    lines = [
        f"{indent}{(name + ':').ljust(width)} {value};" for name, value in pairs
    ]
    return "\n".join(lines)
...
def render_swift(tokens: dict) -> str:
    ...
    material = tokens["material"]
    space = {name: value for name, value in tokens["space"].items() if not name.startswith("_")}
    ...
    lines += [
        ...
        f"        /// The measure every console page shares out of the box.",
