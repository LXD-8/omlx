...
在 `/admin` 提供实时监控、模型管理、聊天、基准测试和模型级设置的 Web UI。支持英语、韩语、日语、中文、法语、俄语、西班牙语和巴西葡萄牙语。所有 CDN 依赖已内置，完全支持离线运行。
...
源码构建可将同一个已下载的语言模型拆分到内存不等的两台 Mac 上运行，基于 MLX 流水线 rank，通过 Ring 或 Thunderbolt RDMA/JACCL 互联。Cluster 仪表盘负责只读节点发现、严格的 SSH/运行时校验、按字节感知的不等分片规划、基于实测的算力与链路再平衡、预留余量的执行调优、一键激活，并在两台 Mac 上实时展示分片与性能映射。交互、均衡、吞吐三种预设档位提供合并批处理、提示缓存亲和、rotating-KV 限制、Ring 连接调优，以及能力门控的实验性纯 token 输出路径。安装步骤、安全边界、当前限制与物理硬件验证清单见[跨 Mac 分布式推理](docs/distributed-cluster.md)。
