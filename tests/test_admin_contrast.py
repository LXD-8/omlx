...
def _text_pairs() -> list[tuple[str, str, str]]:
    ...
    for appearance in APPEARANCES:
        for token in ("primary", "secondary", "tertiary", "muted", "danger", "link"):
...
def _fixed_pairs() -> list[tuple[str, str, str, str]]:
    ...
    pairs += [
        (
            f"{appearance} text.muted on bgTertiary",
            {"light": "#94a3b8", "dark": "#52525b"}[appearance],
            _surface(appearance, "bgTertiary"),
            _text(appearance, "muted"),
        )
        for appearance in APPEARANCES
    ]
    pairs.append(
