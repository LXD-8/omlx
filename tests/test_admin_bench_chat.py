...
def test_the_drawer_carries_the_profile_picker_and_model_settings():
    ...
    assert '@keydown.tab="trapDialogFocus($event)"' in drawer
    ...
    # Esc closes it, after the sheets that own the key.
    handler = _section(CHAT, "handleGlobalKeydown(e) {", "focusChatSearch() {")
    assert "if (this.rightSidebarOpen) { this.closeRightDrawer(); return; }" in handler
...
def test_the_header_keeps_the_summary():
    ...
    assert "activePromptProfile || window.t('chat.profile_default')" in header, (
        "the active profile stays in the header"
    )
    assert "openRightDrawer('profile')" in header
...
def test_shortcut_sheet_lists_only_real_bindings():
    ...
    assert "shortcut-row__key" in sheet
    assert ".shortcut-row__key {" in COMPONENTS_CSS

    listed = set(re.findall(r'class="shortcut-row__key">([^<]+)</kbd>', sheet))
    assert listed == {
    ...
    focus = _section(CHAT, "focusChatSearch() {", "openRightDrawer(tab = null) {")
