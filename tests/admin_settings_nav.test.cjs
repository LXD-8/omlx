...
test('a section counts as current only once the sticky bar clears it', () => {
    // A section becomes current when its top is within ACTIVE_OFFSET of the
    // viewport top — that is, when the sticky bar no longer covers it. One
    // pixel of scroll either side of that line flips the rail.
    const offsets = [0, 400, 900];
    ...
    assert.equal(nav.activeSection(offsets, boundary - 1, SECTIONS), 'settings-language');
});
